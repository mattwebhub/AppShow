...
struct AgentToolResultTests {
  ...
  @Test func timelineSummaryOfAFreshProjectHasOneFullSliceAndNoEffects() throws {
    ...
    #expect(timeline["captions"]?["enabled"] == false)
    #expect(timeline["captions"]?["count"] == 0)
    #expect(timeline["captions"]?["segments"] == [])
    #expect(timeline["captions"]?["style"]?["fontFamily"] == "System")
    #expect(timeline["captions"]?["style"]?["textColor"] == ["r": 1, "g": 1, "b": 1, "a": 1])
    #expect(timeline["captions"]?["style"]?["backgroundColor"] == ["r": 0, "g": 0, "b": 0, "a": 1])
