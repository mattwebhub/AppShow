# AppShow documentation

Start with the [project introduction](../README.md#get-started), then follow the part of the workflow you need.

| Guide | What it covers |
| --- | --- |
| [Recording](recording.md) | Capture modes, webcam and voice options, devices, permissions |
| [Editor](editor.md) | Cuts, music, camera regions, captions, overlays, and the assistant |
| [Export](export.md) | Formats, presets, quality, and rendering |
| [Project format](project-format.md) | Portable `.appshow` bundles and legacy compatibility |
| [Brand assets](brand/README.md) | Artwork, app icons, and repository graphics |

## For contributors

- [Contribution guide](../CONTRIBUTING.md)
- [Architecture overview](architecture/00-overview.md) and [module map](architecture/01-module-map.md)
- [Concurrency](architecture/02-concurrency.md) and [data flow](architecture/03-data-flow.md)
- [Dependencies](architecture/04-dependencies.md), [coding patterns](architecture/05-coding-patterns.md), and [conventions](architecture/06-conventions-checklist.md)
- [Testability](architecture/07-testability.md) and [test strategy](../planning/tdd-strategy.md)
- [Release preparation and publication workflow](../planning/releases/RELEASE-WORKFLOW.md)
- [Public-release checklist](../planning/releases/PUBLIC-RELEASE-CHECKLIST.md) and [Mac App Store readiness evaluation](../planning/releases/APP-STORE-EVAL.md)
- [Current state](../planning/STATE.md) and [architecture decisions](../planning/decisions/README.md)

The feature spikes under `features/` document earlier exploration. Use the current state and milestone verification files to distinguish implemented behavior from ideas and pending manual checks.

- [Store implementation and local verification](../planning/releases/STORE-IMPLEMENTATION.md)
- [Store privacy audit](../planning/releases/PRIVACY-AUDIT.md)
- [Privacy policy](../PRIVACY.md) and [website publishing](../site/README.md)
- [Store dependency evidence](../planning/releases/STORE-DEPENDENCIES.md)
