...
Paths: `Reframed/...` and `ReframedTests/...` are relative to the repository root. `Toone/...` and `TooneTests/...` are relative to `<toone-repo>/apps/toone-desktop/Toone`. Every path and line below was read on the date above.
