...
| Fact | Evidence |
| --- | --- |
| All editable state is stored `var`s on one `@MainActor @Observable final class EditorState` | `AppShow/Editor/EditorState.swift:7-125` |
| Mutations are plain property writes or small methods in `EditorState+*.swift`; nothing is a command object | `AppShow/Editor/EditorState+VideoRegions.swift:5-88`, `+SpotlightRegions.swift:11-119`, `+Zoom.swift:5-103`, `+CameraRegions.swift:115-211`, `+AudioRegions.swift:22-90`, `+Playback.swift:45-52` |
...
| A snapshot is `EditorStateData` (`AppShow/Project/ProjectMetadata.swift:463-495`), produced by `createSnapshot()` (`+Persistence.swift:16-128`) and applied by `restoreFromSnapshot(_:)` (`:130`); `undo()`/`redo()` at `:322-330` | same |
| `History` keeps 50 `HistoryEntry(snapshot:timestamp:)` values with no label field (`AppShow/Editor/History.swift:3-6, 24-35`); labels are computed after the fact by diffing two snapshots (`History+ChangeRules.swift:345-349`) | same |
...
| Keyboard undo/redo goes through `EditorWindow.setupKeyboardMonitor` → `state.undo()/redo()` | `AppShow/Editor/EditorWindow.swift:98-117` |
| One editor window per open project; `SessionState.editorWindows` holds them | `AppShow/State/SessionState+Project.swift:23-56` |
| `.frm` opens arrive through `AppDelegate.application(_:open:)` → `session.openProject(at:)` | `AppShow/App/AppDelegate.swift:100-104` |

### 1.2 Data the agent can inspect

| Data | Where | Notes |
| --- | --- | --- |
| Word-level transcript | `CaptionSegment.words: [CaptionWord]?` with `word/startSeconds/endSeconds` (`ProjectMetadata.swift:154-166`); produced with `wordTimestamps: true` (`AppShow/Utilities/TranscriptionService.swift:48`) and drift-corrected in `EditorState+Captions.swift:37-58` | Transcription is async on WhisperKit; a tool must await `isTranscribing == false` |
| Cursor clicks and samples | `CursorClickEvent(t:x:y:button:)`, `CursorSample`, `KeystrokeEvent` (`AppShow/Editor/CursorMetadata.swift:45-96`); `EditorState.cursorMetadataProvider` (`EditorState.swift:47`) | Normalized 0–1 coordinates |
| Click clusters | `ZoomDetector.detect(from:duration:config:)` groups clicks by dwell (`AppShow/Editor/ZoomDetector.swift:10`); `groupZoomRegions(from:)` turns keyframes into regions | Pure, T1 |
| Audio samples for silence detection | `AudioWaveformGenerator.extractSamples` is `nonisolated private static` (`AppShow/Editor/AudioWaveformGenerator.swift:33`) | Seam S7 (drop `private`) or a new `SilenceDetector` |
| Preview/export geometry | `EditorState.canvasSize(for:)` (`+CameraLayout.swift:52`), `ExportConfiguration` (`AppShow/Compositor/ExportConfiguration.swift:7-68`) | |

### 1.3 Rendering paths a new primitive must touch

`FrameRenderer.renderFrame` (`AppShow/Compositor/FrameRenderer.swift:190`) draws, in order, `+Background` → `+Screen` → `+Webcam` → `+Cursor` (`FrameRenderer+Cursor.swift:5`) → `+Spotlight` (`FrameRenderer+Spotlight.swift:5`) → `+Captions` (`FrameRenderer+Captions.swift:6`, called at `FrameRenderer.swift:320-330`). Every parameter comes from the immutable `CompositionInstruction` (`AppShow/Compositor/CompositionInstruction.swift:33-171`) built in `VideoCompositor+InstructionBuilder.swift`. The preview mirrors each stage with CALayers (`VideoPreviewView+Update.swift:200 updateOverlays`, `VideoPreviewContainer+Spotlight.swift:5`, `SpotlightOverlayLayer.swift:34`). `06-conventions-checklist.md` §"Adding a new editor property" lists the 12 sites; a new overlay primitive hits all of them plus a timeline track modeled on `TimelineView+SpotlightTrack.swift:4-193`.

### 1.4 What Toone already solved (reuse)

| Concern | Toone evidence | Reuse |
| --- | --- | --- |
| MCP server shape: `Server` from `@modelcontextprotocol/server` 2.0.0 with `setRequestHandler("tools/list")` / `("tools/call")`, served over stdio | `<toone-repo>/packages/mcp-server/src/index.ts:1149-1162, 1799-1804`; `package.json` | Same JSON-RPC methods, whichever language hosts them |
...
| Signal | Mechanism |
| --- | --- |
| "Agent is editing" badge | new `EditorState.agentActivity: AgentActivity?` (`struct {label, startedAt, batchDepth}`) rendered as a pill in `EditorTopBar` (`AppShow/Editor/EditorTopBar.swift:3-32`) using `AppShowColors` and `FontSize` tokens; deliberately not listed in `observeChanges()` so it never triggers a save or snapshot |
...
- Canonical source: `AppShow/Agent/Skills/<name>/SKILL.md` in the repo, copied into `AppShow.app/Contents/Resources/Skills/` at build time as a folder resource.
...
| 7.5 | Silence detection | none persisted; `SilenceDetector.detect(samples:sampleRate:thresholdDb:minGap:) -> [(start,end)]` in `AppShow/Editor/SilenceDetector.swift` (T1) over `AudioWaveformGenerator.extractSamples` (S7) | n/a | n/a | n/a | results become keep-slices (spike 01) or, until then, `videoRegions` |
...
```
 chat panel (spike 03)     runtime child process        reframed-mcp shim        AppShow.app
