...
struct ReframedProjectTests {
  private func makeProject(
    ...
    webcam: Bool = true,
    cursor: Bool = true
  ) async throws -> (ReframedProject, RecordingResult) {
  ...
  private func readMetadata(_ project: ReframedProject) throws -> ProjectMetadata {
  ...
  @Test func openRestoresMetadataAndOptionalMedia() async throws {
    ...
    let opened = try ReframedProject.open(at: created.bundleURL)
  ...
  @Test func openPrefersScreenMovOverScreenMp4() async throws {
    ...
    #expect(try ReframedProject.open(at: created.bundleURL).screenVideoURL.lastPathComponent == "screen.mp4")
    try Data().write(to: created.bundleURL.appendingPathComponent("screen.mov"))
    #expect(try ReframedProject.open(at: created.bundleURL).screenVideoURL.lastPathComponent == "screen.mov")
  }
  ...
  @Test func openWithoutMediaReportsNilOptionalFiles() async throws {
    ...
    let project = try ReframedProject.create(from: result, fps: 30, captureMode: .selectedArea, in: dir, cleanupTemp: false)
    let opened = try ReframedProject.open(at: project.bundleURL)
  ...
  @Test func openThrowsWhenScreenRecordingIsMissing() async throws {
    ...
    #expect(throws: CaptureError.self) {
      try ReframedProject.open(at: project.bundleURL)
    }
    do {
      _ = try ReframedProject.open(at: project.bundleURL)
  ...
  func createWithDefaultCleanupWipesSharedTempDir() async throws {
    ...
    _ = try ReframedProject.create(from: result, fps: 30, captureMode: .entireScreen, in: dir)
    #expect(!FileManager.default.fileExists(atPath: sentinel.path))
  }
}
