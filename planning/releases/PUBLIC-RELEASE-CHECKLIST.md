...
- [ ] **D1 — Engineering:** choose the AppShow release version and unique build number. Correct `AppShow/Info.plist` to resolve `CFBundleVersion` from `CURRENT_PROJECT_VERSION`; align `sparkle:version` in `scripts/generate-appcast.sh` with the compiled build number. Both currently use the marketing version even though build 26 is configured. Evidence: packaged Info.plist and appcast agree, with monotonically increasing versions. [Sparkle version contract](https://sparkle-project.org/documentation/publishing/)
- [ ] **D2 — Engineering:** make a clean Release build and verify `arm64 x86_64` if both are advertised. Inspect every embedded framework/helper and dependencies. Evidence: `make release`, `lipo -archs`, release logs, bundle contents and deployment targets. An old DMG in `dist/` is not the candidate.
