...
- [x] T7. Blur region primitive (phase 8; `planning/features/blur-regions/`). Proof: persistence/state/history tests, trim and cut remapping, compositor selection, source-space golden frames with and without zoom, preview mirror, shared Overlays timeline chip, Effects controls, and a warning-free build.
- [x] T8. Transitions on overlays and keep-slices. Proof: `RegionRemappingTests`, golden frames at transition midpoints, and the typed `set_transition` agent tool.
- [x] T9. Docs and divergences: `planning/upstream-sync.md`, `docs/architecture/07-testability.md` if a seam is added. Proof: grep.
