...
- [x] T1. JSON value and JSON-RPC codec. Proof: `AppShowTests/Agent/JSONValueTests.swift`, `JSONRPCCodecTests.swift`.
...
- [x] T7. VERIFY.md run, branch pushed, PR opened. Proof: https://github.com/mattwebhub/AppShow/pull/7.
...
- `sun_path` limit (104 bytes) on long project folders: socket path falls back to `AppShowPaths.temp`; `session.json` is the source of truth.
- Concurrent milestone 04 worktree adds `AppShow/Agent/` and `AgentJSONValue`: this milestone uses `AppShow/Agent/Tools/`, the `JSONValue` name, and pbxproj ids `7E57…B0` to `CF`; the merge conflict is limited to adjacent lines in the pbxproj.
