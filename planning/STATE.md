...
- Milestone 06 integrates milestones 03–05 and milestone 07's completed primitives on `milestone-06-agent-tools-editing`; labeled history, rollback, exact cuts, grouped transactions, single-use confirmations, provider-scoped MCP configuration, the signed stdio shim, bridge lifecycle, live activity UI, presentation settings, timed captions, exact-path export, silence removal, and text/image tools are implemented test-first. Provider E2E remains.
...
- Milestone 06 integrated editing surface: format, lint, build, and 643 tests in 71 suites pass; 4 gated export tests and the gated real-process shim test pass.
...
1. Complete milestone 06 by exercising Claude Code and Codex end to end through the signed shim.
