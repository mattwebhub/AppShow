...
## Direct-release foundation

Milestone 18 implements independent compiled build numbering, built-bundle appcast metadata, Ed25519 archive verification against the app public key, and separate prepare/tag/preview/publish commands. Preparation records clean source and artifact hashes; publication validates the existing receipt and tag. The current manual-download behavior remains provisional while the release update preference is pending.

Verification: 782 tests in 93 suites, formatting, lint and warning-free Debug build pass. The universal Release build succeeds for Intel and Apple Silicon with version `0.14.7` and build `26`; 11 missing module-cache warnings remain for follow-up. See [verification](milestones/18-release-foundation/VERIFY.md) and [release workflow](releases/RELEASE-WORKFLOW.md). Changes are committed locally in small semantic groups. No real candidate, tag or publication was created. Both distribution channels remain NOT READY pending version selection, real signing/installation QA and store-specific work.

## App Store foundation

Milestone 19 adds the independent `AppShowStore` target/module, sandbox entitlements, container paths, persistent folder/project bookmarks and scopes held across media operations. It excludes Sparkle/updater UI and the MCP helper, disables the external assistant in the validation edition pending the shipping design decision, skips Accessibility and keeps local shortcuts/visible recording controls. Both app variants now declare executable/application-type metadata. Store-only privacy API reasons, dependency evidence and a repeatable artifact evaluator are checked in.
