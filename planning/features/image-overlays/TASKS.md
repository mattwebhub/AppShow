...
- [x] 4. Preview: cached `CGImage` per filename, wrapper/content layer split for unclipped shadows and rounded image bounds, shared layout and transition math, wired through `VideoPreviewView` and `EditorView+Preview`; format, lint, and build warning-free.
