...
2. The first split reveals a Cuts track directly under Screen, animated in the same way the Zoom and Spotlight tracks appear. Each kept slice is a region with draggable edges and body; gaps are drawn dimmed. Clicking a slice selects it; Delete/Backspace, the transport trash button, or its popover removes it with one immediate Undo step. The view closes the removed gap while the original recording remains unchanged. Dragging a shared boundary adjusts both adjacent slices without removing footage. Removing the last cut animates the track out.
...
6. Compressed timeline mode (toggle visible only when cuts exist) hides gaps from the ruler and all tracks; zoom regions remain movable and resizable in that mode; the remaining effect and audio tracks are read-only.
...
- Editing audio, camera, spotlight, and overlay tracks in compressed mode.
