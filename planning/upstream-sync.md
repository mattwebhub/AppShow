...
- Agent chat (milestone 04, phases 1–3): `Reframed/Project/ReframedProject.swift` gains `agentDirectory`; new `Agent` group in `project.pbxproj` (ids `7E5700000000000000000060`–`78`) with `AgentProvider.swift`, `AgentEvent.swift`, `AgentJSONValue.swift`, `AgentError.swift`, `ClaudeCodeProvider.swift`, `CodexProvider.swift`, `AgentProcessRunner.swift`, `AgentToolchain.swift`, `AgentThreadData.swift`, `AgentThreadStore.swift`, `AgentSession.swift`, `AgentTranscript.swift`: `docs/features/03-agent-chat/ATTACK-PLAN.md`
