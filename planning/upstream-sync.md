...
- Silence removal (milestone 07, first primitive): `Reframed/Editor/History.swift` gains `HistoryEntry.label` (optional, omitted from JSON when nil) and `pushSnapshot(_:label:)`; `Reframed/Editor/HistoryPopover.swift` shows the label before the derived description; `Reframed/Editor/PropertiesPanel.swift` adds `silencesSection` to the Video tab; new files `Reframed/Editor/SilenceDetector.swift`, `SilenceAnalysis.swift`, `EditorState+SilenceRemoval.swift`, `PropertiesPanel+SilencesSection.swift` registered in `project.pbxproj` with ids `7E5700000000000000000080`–`87`: `planning/features/silence-removal/SPEC.md`
- Text overlays (milestone 07, second primitive): `Reframed/Project/ProjectMetadata.swift` gains `EditorStateData.textOverlays`; `Reframed/Editor/EditorState.swift`, `EditorState+Persistence.swift`, `EditorState+Export.swift`, `History+ChangeRules.swift` carry the field; `Reframed/Compositor/CompositionInstruction.swift` (`TextOverlayInstruction`), `ExportConfiguration.swift`, `VideoCompositor+InstructionBuilder.swift`, `VideoCompositor+RegionRemapping.swift` (`remapTextOverlay`), `FrameRenderer.swift`, `FrameRenderer+HDR.swift` (one `drawTextOverlays` call before captions); preview mirror in `VideoPreviewView(+Update).swift`, `VideoPreviewContainer.swift`, `EditorView+Preview.swift`; track gating in `TimelineView.swift`, `EditorView.swift`; `PropertiesPanel.swift` adds `overlaysSection`; new files `TextOverlayData.swift`, `EditorState+TextOverlays.swift`, `TextOverlayLayout.swift`, `FrameRenderer+TextOverlays.swift`, `VideoPreviewContainer+TextOverlays.swift`, `TimelineView+OverlayTrack.swift`, `TextOverlayEditPopover(+Style).swift`, `PropertiesPanel+OverlaysSection.swift` (pbxproj ids `7E5700000000000000000090`–`A1`): `planning/features/text-overlays/SPEC.md`
- Image overlays (milestone 07, third primitive): project metadata, editor state, persistence, history, export configuration, compositor instructions/remapping/renderers, preview views, shared Overlays track and Effects properties are extended; new model/importer/editor/layout/renderer/preview/chip/popover/position-control files are registered with pbxproj ids `7E57000000000000000000A2`–`B3`: `planning/features/image-overlays/SPEC.md`
- Blur regions (milestone 07, fourth primitive): project metadata, editor state, persistence, history, export configuration, region remapping, compositor selection, SDR/HDR source rendering, preview views, shared Overlays track and Effects properties are extended; new model/editor/renderer/preview/timeline/popover files use pbxproj ids `7E5D000000000000000000D1`–`DC`: `planning/features/blur-regions/SPEC.md`
