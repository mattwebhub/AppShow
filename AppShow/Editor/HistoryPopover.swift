...
struct HistoryPopover: View {
    ...
    -> some View
  {
    ...
    return Button {
      ...
      HStack(spacing: 8) {
        Circle()
          .fill(isCurrent ? AppShowColors.primaryText : AppShowColors.border)
        ...
        VStack(alignment: .leading, spacing: 2) {
          if index == 0 {
            Text("Initial state")
              ...
              .foregroundStyle(
                isFuture
                  ? AppShowColors.tertiaryText
                  : isCurrent ? AppShowColors.primaryText : AppShowColors.secondaryText
              )
          } else {
      ...
      .background(
        RoundedRectangle(cornerRadius: Radius.md)
          .fill(isCurrent ? AppShowColors.selectedBackground : Color.clear)
      )
